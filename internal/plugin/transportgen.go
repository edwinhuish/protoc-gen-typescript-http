...
import (
	...
	"github.com/edwinhuish/protoc-gen-typescript-http/internal/codegen"
	"github.com/edwinhuish/protoc-gen-typescript-http/internal/httprule"
...
func generateTransportInfra(f *codegen.File, defaultHost string) {
	...
	f.P(t(1), "unary(path: string, method: string, data: Record<string, unknown> | null, meta: TransportMeta): Promise<unknown>;")
	...
	f.P()
	f.P(t(1), "return {")
	f.P(t(2), "unary(path, method, data, _meta) {")
	f.P(t(3), "const body = data ? JSON.stringify(data) : undefined;")
	f.P(t(3), "const init: RequestInit = { method, body };")
